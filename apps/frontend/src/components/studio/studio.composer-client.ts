...
// 'smart' (default) = scene-aligned best window; 'head' = first N seconds.
export type CutStrategy = 'smart' | 'head';
export interface VideoAdCut { id: string; url: string; channelId: string; length: number; w: number; h: number; start?: number; strategy?: CutStrategy; reason?: 'scene' | 'head' }
...
export function composeVideoAd(payload: {
  adId?: string; clipRef: string; channels: string[]; lengths: number[]; copy?: ComposeCopy; brandKitId?: string; addToAd?: boolean; music?: MusicSpec; strategy?: CutStrategy;
}): Promise<{ brandKitId: string; cuts: VideoAdCut[]; skipped: VideoAdSkip[]; sourceDuration: number; music?: string | false; strategy?: CutStrategy; sceneCount?: number }> {
